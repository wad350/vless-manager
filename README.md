...
- встроенный Xray core без отдельного процесса;
- VLESS Reality/TLS с транспортами TCP, WebSocket, gRPC, HTTP Upgrade и XHTTP;
...
- структурированный журнал manager и Xray;
...
OpenWrt требует отдельной адаптации маршрутизации и пакета.
...
Готовый пакет появится в `build/`. В бинарник встраиваются WebUI и Xray
`v26.9.9` (последний prerelease на 25.09.2026) с локальными исправлениями;
отдельный пакет Xray не требуется.
...
curl -fsSL https://raw.githubusercontent.com/wad350/vless-manager/main/install.sh | \
  VLESS_MANAGER_VERSION=1.16.7 sh
```
...
По умолчанию используется `root@192.168.13.1:222`. Другой адрес:
...
```sh
GOCACHE="$PWD/.gocache" GOTOOLCHAIN=go1.27.1 \
  go test ./...
```
...
- `xray_src/` — зафиксированный исходный код Xray и перечень локальных исправлений;
- `packaging/` — сборка пакетов Entware и init-скрипт;
...
- используется embedded Xray core с поддержкой XHTTP;
...
- перед первой миграцией файлы `config.json` и `subscriptions.json` сохраняются
  рядом с суффиксом `.before-xray`.
