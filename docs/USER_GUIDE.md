...
3. **Установить обновление** запускает фоновую загрузку MIPSLE-бинарника и
   отдельного файла SHA-256.
4. Центр обновления показывает этапы, общий процент, загруженный объём,
   скорость и фактически выбранный маршрут.
5. Менеджер проверяет контрольную сумму и ELF-архитектуру, затем перезапускает
   сервис. Страница ожидает его появления и подтверждает новую версию.
6. Если новый процесс не запустился, init-скрипт возвращает предыдущий
   бинарник.
