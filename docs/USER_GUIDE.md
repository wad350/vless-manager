...
3. **Обновить** загружает MIPSLE-бинарник и отдельный файл SHA-256.
4. Менеджер проверяет контрольную сумму и ELF-архитектуру, затем перезапускает
   сервис.
5. Если новый процесс не запустился, init-скрипт возвращает предыдущий
   бинарник.
