...
flowchart LR
    ...
    RULES -->|"внешний TCP и UDP"| TUN["tun0 / sing-box"]
...
`tcp`, `ws`, `grpc`, `http`/`h2`, `httpupgrade`, `quic` и `xhttp`.
XHTTP обслуживает встроенный sing-box extended.
...
При этом sing-box встроен в тот же процесс, а тяжёлые операции выполняются
...
Проверенные конфигурации — Keenetic OS с Entware `mipsel-3.4` на MT7621 и
`aarch64-3.10` на ARM64.

- TUN MTU фиксирован на `1500`;
- Go toolchain фиксирован на `1.26.4`;
- sing-box фиксирован на `v1.13.18-extended-2.6.5`;
...
- XHTTP обслуживается встроенной extended-сборкой.
