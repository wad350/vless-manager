...
type systemDialerKey struct{}
type systemDialerFeatures struct {
	dns       dns.Client
	outbounds outbound.Manager
	done      <-chan struct{}
}

var defaultDialerFeatures atomic.Pointer[systemDialerFeatures]

// NewSystemDialerContext isolates embedded instances, including features that
// capture their context before all of the instance's dependencies are built.
func NewSystemDialerContext(ctx context.Context) (context.Context, func(dns.Client, outbound.Manager)) {
	features := &systemDialerFeatures{done: ctx.Done()}
	return context.WithValue(ctx, systemDialerKey{}, features), func(dc dns.Client, om outbound.Manager) {
		features.dns, features.outbounds = dc, om
	}
}

// ContextWithSystemDialerFeatures carries the instance's DNS and outbound
// manager through transports whose networking library creates a new context.
func ContextWithSystemDialerFeatures(dst, src context.Context) context.Context {
	if features, ok := src.Value(systemDialerKey{}).(*systemDialerFeatures); ok {
		return context.WithValue(dst, systemDialerKey{}, features)
	}
	return dst
}

func SystemDialerDone(ctx context.Context) <-chan struct{} {
	if features, ok := ctx.Value(systemDialerKey{}).(*systemDialerFeatures); ok {
		return features.done
	}
	return nil
}

func dialerFeaturesFromContext(ctx context.Context) *systemDialerFeatures {
	if features, ok := ctx.Value(systemDialerKey{}).(*systemDialerFeatures); ok {
		return features
	}
	if features := defaultDialerFeatures.Load(); features != nil {
		return features
	}
	return &systemDialerFeatures{}
}
...
func LookupForIPContext(ctx context.Context, domain string, strategy DomainStrategy, localAddr net.Address) ([]net.IP, error) {
	dnsClient := dialerFeaturesFromContext(ctx).dns
	if dnsClient == nil {
...
func DialSystem(ctx context.Context, dest net.Destination, sockopt *SocketConfig) (net.Conn, error) {
	...
	if len(sockopt.DialerProxy) > 0 {
		obm := dialerFeaturesFromContext(ctx).outbounds
		if obm == nil {
...
func InitSystemDialer(dc dns.Client, om outbound.Manager) {
	defaultDialerFeatures.Store(&systemDialerFeatures{dns: dc, outbounds: om})
}
